...
void Init() {
    gEnabled.store(EnvOn("GA_TEXT_PROBE") || FlagFileOn("ga_text_probe.enable"));
    ...
    if (gEnabled.load()) {
        ...
        x::runtime::LogI("GaTextProbe", "disabled (set GA_TEXT_PROBE=1 or state/ga_text_probe.enable)");
    }
}
