...
//   InputSystem.QueueEvent(InputEventPtr)      @0x47321C0
//   Keyboard.get_current()                     @0x478d6b0
//   InputSystem.get_settings()                 @0x4732870
//   InputSettings.set_backgroundBehavior(e)    @0x47B0C40
//   InputSystem.EnableDevice(InputDevice)      @0x4730EB0
...
constexpr uint32_t kRvaQueueEvent = 0x47321C0;
constexpr uint32_t kRvaKeyboardGetCurrent = 0x478d6b0;
constexpr uint32_t kRvaGetSettings = 0x4732870;
constexpr uint32_t kRvaSetBackgroundBehavior = 0x47B0C40;
constexpr uint32_t kRvaEnableDevice = 0x4730EB0;
// InputControl.get_currentStatePtr() → InputStateBuffers.GetFrontBufferForDevice(deviceIndex)@0x48061C0
// 已在运行期 IDB（imagebase 0x7ff848c80000 → VA 0x7FF84D37D900）反汇编核实。
constexpr uint32_t kRvaGetCurrentStatePtr = 0x4735e90;
...
constexpr uint32_t kRvaKeyboardPreProcess = 0x478f5b0;
// InputManager 的事件循环按设备标志位 DeviceFlags.HasEventPreProcessor(0x4000) 决定要不要调
// pre-processor。这位是 0 时，钩子装得再对也永远不会被调到 —— 必须实读，必要时置上。
constexpr uint32_t kRvaGetHasPreProc = 0x47415c0;
constexpr uint32_t kRvaSetHasPreProc = 0x47415d0;
