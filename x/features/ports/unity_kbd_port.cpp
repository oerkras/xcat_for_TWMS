...
//   InputSystem.QueueEvent(InputEventPtr)      @0x47334B0
//   Keyboard.get_current()                     @0x478e9a0
//   InputSystem.get_settings()                 @0x4733B60
//   InputSettings.set_backgroundBehavior(e)    @0x47B1F30
//   InputSystem.EnableDevice(InputDevice)      @0x47321A0
...
constexpr uint32_t kRvaQueueEvent = 0x47334B0;
constexpr uint32_t kRvaKeyboardGetCurrent = 0x478e9a0;
constexpr uint32_t kRvaGetSettings = 0x4733B60;
constexpr uint32_t kRvaSetBackgroundBehavior = 0x47B1F30;
constexpr uint32_t kRvaEnableDevice = 0x47321A0;
// InputControl.get_currentStatePtr() → InputStateBuffers.GetFrontBufferForDevice(deviceIndex)@0x48074B0
// 已在运行期 IDB（imagebase 0x7ff848c80000 → VA 0x7FF84D37D900）反汇编核实。
constexpr uint32_t kRvaGetCurrentStatePtr = 0x4737180;
...
constexpr uint32_t kRvaKeyboardPreProcess = 0x47908a0;
// InputManager 的事件循环按设备标志位 DeviceFlags.HasEventPreProcessor(0x4000) 决定要不要调
// pre-processor。这位是 0 时，钩子装得再对也永远不会被调到 —— 必须实读，必要时置上。
constexpr uint32_t kRvaGetHasPreProc = 0x47428b0;
constexpr uint32_t kRvaSetHasPreProc = 0x47428c0;
