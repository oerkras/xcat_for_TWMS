...
| 背景行为 | 默认 `backgroundBehavior=IgnoreFocus` + `InputSystem.runInBackground=true`；失焦每 1.5s 重设并 EnableDevice（`XCAT_KBD_BG=0` 可关） |
...
| 失焦走路中途站住，键「按着」 | 门闩边沿被吞后同态补写永不触发；`RefreshWalkEdgeOnMain` 同帧松→按两条事件游戏吃成 `ma=9→3→9`，人仍不动（BIN 2026-09-09 13:59:37 / 13:59:48 fg=0），反向退一步这种**跨帧**真边沿才走得动 | 补边沿改跨帧：本帧只松（`gEdgePending`，tick 保住），≥32ms 后 `RepushOnMain` 按回；同向再灌（失焦 120ms relatch）落在松着的窗内不提前按回，换向 / 松手则新命令优先 |
