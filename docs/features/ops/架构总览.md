...
```mermaid
flowchart TD
  APP["xcat_app  xcat.exe / ImGui"]
  WEB["msc_webview_login  同进程换票 CDP/HTTP"]
...
4. 换票走 **同进程** `msc_webview_login`（GAMA PASS CDP / HTTP Beanfun）；**无 WebView2**，无独立 `MscLauncher.exe`。
...
| 启动：GAMA PASS / HK 换票 → NGM 或接管 → Classic `LoadLibrary` | [`GAMA_PASS与注入闭环.md`](GAMA_PASS与注入闭环.md)、[`GAMA_PASS账号卡UIA点选.md`](GAMA_PASS账号卡UIA点选.md)、[`启动系统实现.md`](启动系统实现.md) |
...
| 启动 | launcher_core 编排 NGM + 注入 | xcat 内嵌：**GAMA PASS CDP** / HTTP → NGM 或接管 → Classic LoadLibrary |
| GUI | 全功能面板 | 启动壳 + 功能 Tab（无 WebView2） |
...
| 目录 | 写什么 |
|------|--------|
| `xcat_app/` | 用户可见 ImGui；含 `lie_ai_pump` / `anti_bot_ai` |
| `launcher/` | `msc_launch` + `msc_webview_login`（CDP/HTTP 换票门面） |
